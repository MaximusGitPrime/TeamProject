...
function generatePack(cards, packType) {
  ...
  var pack = [];
  var regularCards = cards.filter(function (card) {
    return card.cardType === "common";
  });
  var specialCards = cards.filter(function (card) {
    return card.cardType === "rare";
...
function handleBackToPacksStore(e) {
  ...
  console.log("Send Button function activated"); // mtn changes to save to local storage -->
  ...
  localStorage.setItem(currentUser + ".cards", JSON.stringify(updatedCards)); // end of changes -->
